...
@testset "Data Reporting System Tests" begin
    ...
    @testset "DataReport Basic Functionality" begin
        ...
        @test !has_issues(report)
        
        # Test adding different types of reports
        add_note!(report, "test", "This is a note", "test_location")
        @test length(report.items) == 1
        @test has_issues(report)
        @test !report.has_errors
        
        add_warning!(report, "test", "This is a warning", "test_location")
        @test length(report.items) == 2
        @test !report.has_errors
        
        add_error!(report, "test", "This is an error", "test_location")
    ...
    @testset "Validation Functions" begin
        ...
        @test !validate_file_exists(report, "nonexistent_file.csv", "test file")
        @test report.has_errors
        ...
        if isfile(test_file)
            @test validate_file_exists(report, test_file, "test file")
            @test !report.has_errors
        end
        ...
        @test validate_required_columns(report, test_df, [:index, :g_max], "test")
        @test !validate_required_columns(report, test_df, [:index, :missing_col], "test")
        
        # Reset report for numeric validation
        report = DataReport()
        
        # Test numeric column validation
        @test validate_numeric_column(report, test_df, :g_max, "test"; positive=true)
        @test !validate_numeric_column(report, test_df, :bad_data, "test")
    end
    ...
    @testset "Plants Data Validation" begin
        ...
        add_plants!(params, valid_plants, report, "test_plants")
        ...
        add_plants!(params, invalid_plants, report, "test_plants")
    ...
    @testset "Integration Test with Sample Data" begin
        ...
        if isdir(datapath)
            ...
            println("\\n=== Sample Data Loading Report ===")
            print_report(report)
            
            # Test backward compatibility
            params_old = load_data(data_files)
            @test length(params_old.sets.P) == length(params.sets.P)
        else
...
println("\\n=== Running Data Reporting System Tests ===")