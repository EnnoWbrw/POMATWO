...
"""
    link_components(sr::SubRun{MT,PS,RD,MS}) where {MT<:ZonalMarketType,PS<:ProsumerSetup,RD<:RedispatchSetup,MS<:TwoDayAhead}

Links all components for a zonal market in the TwoDayAhead basecase market state.
Since TwoDayAhead uses DC load flow (nodal model), this uses nodal energy balance with NETINPUT.
Stores results in the :NodalMarketBalance DataFrame.
"""
function link_components(sr::SubRun{MT,PS, RD, MS}) where {MT<:ZonalMarketType,PS <: ProsumerSetup, RD <: RedispatchSetup,MS<:TwoDayAhead}
